...
{
    int i,P[2],PingWei[3];
    int Sum_PingWei[2]={0,0},Sum_Pingwei_Ticket=0;
    ...
    for(i=0;i<3;i++)
    {
        ...
        Sum_Pingwei_Ticket+=PingWei[i];
    }

    if(Sum_Pingwei_Ticket==3)
    {
        printf("The winner is b: %d + 3",P[1]);
    }
    else if (Sum_Pingwei_Ticket==0)
    {
    ...
    else
    {
        if(P[0]>P[1])
        {
            printf("The winner is a: %d + %d",P[0],Sum_PingWei[0]);
        }
        else
        {
            printf("The winner is b: %d + %d",P[1],Sum_PingWei[1]);
        }
    }
    return 0;   
}