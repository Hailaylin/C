...
int main()
{
    ...
    for(i=0;i<STU;i++)
    {
        ...
        StuAverageScore[i]=StuSumScore[i]/5.0;
    }
/*  测试内容
    for(i=0;i<STU;i++)
    {
    ...
    for(i=0;i<STU;i++)
    {
        printf("%.2f\t",StuAverageScore[i]);
    }
*/
    
    for(i=MaxStuOrder=0;i<STU-1;i++) 
    {
        if(StuAverageScore[MaxStuOrder]<StuAverageScore[i+1])
        {
            MaxStuOrder=i+1;
            //MaxStuOrder=i+1;
            /*
    ...
    printf("stu_order=%d\nmax=%7.2f",MaxStuOrder,StuAverageScore[MaxStuOrder]);
    return 0;
}
